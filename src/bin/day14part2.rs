...
fn main () {
    ...
    for i in 0..max_height+3 {
        let mut row = Vec::<Material>::new();
        for j in min_width..max_width+1+(2*offset) {
    ...
    for j in 0..grid[0].len() {
        grid[(max_height + 2) as usize][j] = Material::Rock;
    }
        ...
        for path in group.windows(2) {
            ...
            for i in start_row..end_row+1 {
                for j in start_col..end_col+1 {
                    grid[i][(j + offset as usize) as usize] = Material::Rock;
                }
            }
        }
    }
    ...
    let mut units = 0;
    'outer: loop {
        let mut curr_col = (500 - min_width + offset) as usize ;
...
fn print_grid(grid: &Vec<Vec<Material>>, min_width: u32) {
    for i in 0..grid.len() {
        for j in 0.. grid[i].len() {
            match (i, j) {
                (0, j) if 500 - min_width as usize + 500 == j => print!("+"),
